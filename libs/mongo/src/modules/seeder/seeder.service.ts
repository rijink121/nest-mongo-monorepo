      ...
      if (Object.prototype.hasOwnProperty.call(item, key)) {
        ...
        if (value instanceof SeedReference) {
          try {
            ...
            const parent: Document | null = await this.connection.models[
              value.model
            ].findOne(value.where);

            // Replace reference with parent document ID or null if not found
            if (parent) body[key] = parent.id as string;
            else body[key] = null;
          } catch (error) {
            this.logger.warn(
    ...
    for (let index = 0; index < seed.data.length; index++) {
      ...
      if (
        ...
        try {
          ...
          const record: Document | null = await this.connection.models[
            seed.model
          ].findOne(seed.alwaysWhere(body));
